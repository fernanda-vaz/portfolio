  ...
  return (
    <div className='flex items-center gap-4 py-8 justify-center max-md:flex-wrap'>
      <Transition viewport={{ once: true }}>
        <button
          className={cn(
            'border-fontColor-50/30 px-6 py-2 rounded-full relative',
        ...
        >
          {filterValue === 'all' && (
            <motion.span
              ...
              className='absolute top-0 left-0 w-full h-full bg-neonGreen-400 -z-10 rounded-full'
            />
          )}
      ...
      {filters.map((filter, index) => (
        ...
        >
          <motion.button
            ...
            className='relative border border-fontColor-50/20 px-3 py-2 rounded-full'
          >
            {filterValue === filter && (
              <motion.span
                transition={{ type: 'spring', bounce: 0.3 }}
